...
public sealed class MagicBytesSignal : IDetectionSignal
{
    ...
    public void Evaluate(DetectionContext context)
    {
        ...
        // OLE2 / Compound File Binary signature. Among supported kinds this means an Outlook
        // .msg file; legacy binary Office documents share it but are not supported. Low
        // confidence so an explicit extension/content-type wins.
        if (bytes.Length >= 8 && bytes[0] == 0xD0 && bytes[1] == 0xCF && bytes[2] == 0x11 && bytes[3] == 0xE0
            && bytes[4] == 0xA1 && bytes[5] == 0xB1 && bytes[6] == 0x1A && bytes[7] == 0xE1)
        {
            context.AddCandidate(DocumentKind.Email, 35, Name);
            return;
        }
